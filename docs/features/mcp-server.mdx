...
The mAItion MCP server exposes the retrieval API — the underlying [rag-of-all-trades](https://github.com/WikiTeq/rag-of-all-trades) (ROAT) endpoints — as a [Model Context Protocol](https://modelcontextprotocol.io/) endpoint. This lets any MCP-compatible system — agents, orchestration frameworks, or custom integrations — call mAItion's retrieval tools programmatically.

<Note>
This grants access to ROAT's retrieval tools only (`retrieve_chunks`, `rephrase_chunks`), not to mAItion's chat models. To have an MCP client talk to mAItion's chat models instead, see [MCP Client](/features/mcp-client).
</Note>
...
By default, the `api` service is internal to the Docker network. MCP clients running on your host machine need port `8000` exposed. Use the provided `compose.mcp.roat.yaml` override — this avoids editing `compose.yaml` directly:

```bash
docker compose -f compose.yaml -f compose.mcp.roat.yaml up -d api --force-recreate
```
...
| Symptom | Cause | Fix |
|---|---|---|
| `Connection refused` on port 8000 | Port not exposed | Use `compose.mcp.roat.yaml` override to expose port 8000 |
