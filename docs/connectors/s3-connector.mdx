...
- reads files from a single bucket per connector instance
...
- `S3_ACCOUNT1_BUCKET`: bucket name
- `S3_ACCOUNT1_SCHEDULES`: ingestion interval in seconds (default is `3600`)
...
## Multiple S3 Buckets or Accounts

Each connector instance targets a single bucket. To ingest multiple buckets, add more `sources`
entries (`account2`, `account3`, etc) with separate env vars per instance.
