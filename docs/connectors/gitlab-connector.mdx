...
Use the GitLab Connector to ingest repository files and, optionally, issues from a GitLab project into the mAItion knowledge base.
...
- optionally ingests the project's issues, with filtering by state, labels, assignee, author, milestone, and more
...
The underlying reader re-downloads all repository file content from the GitLab API on every scheduled
run. The vector store's checksum-based deduplication still prevents re-embedding unchanged files, but it
does not reduce GitLab API calls for the files connector.
...
| `project_id` | yes | — | Integer project ID to ingest files (and optionally issues) from |
