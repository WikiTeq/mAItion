...
`summary_model`, `keep_first`, `keep_last`, and `compression_style` can be edited after install from the filter's settings in mAItion. The other 5 valves in the table above are set at install time from the env vars below, and can also be edited afterward from the filter's settings.
